    ...
    public class SetProperty
    {
        ...
        public static Block? SetBlock(Type type, BlockModel blockModel)
        {
            ...
            if (constructorInfo != null)
            {
                ...
                if (blockModel.flagPresenceСomment)
                    block.SetComment(blockModel.textOfComment);
            }
            return block;
        }
        ...
        public static CaseBlock? SetCaseBlock(CaseModel caseModel)
        {
            ...
            if (caseModel.nameOfBlock == "CaseFirstOption")
                caseBlock = new CaseFirstOption(new Edblock(), 0, caseModel.countOfLine);
            else if (caseModel.nameOfBlock == "CaseSecondOption")
                caseBlock = new CaseSecondOption(new Edblock(), 0, caseModel.countOfLine);
            if (caseBlock != null)
            {
                UIElement uIElementOfСase = caseBlock.GetUIElement();
                if (caseBlock.TextBox != null)
                    caseBlock.TextBox.Text = caseModel.textOfBlock.ToString();

                Canvas.SetTop(uIElementOfСase, caseModel.topCoordinates);
                Canvas.SetLeft(uIElementOfСase, caseModel.leftCoordinates);

                if (caseModel.flagPresenceСomment)
                    caseBlock.SetComment(caseModel.textOfComment);
            }
            return caseBlock;
        }
    }
